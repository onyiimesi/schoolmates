...
class ResultResource extends JsonResource
{
    ...
    {
        $meta = $this->presenter->getMetadata(
            $this->resource,
            $this->class_name,
        );
    ...
    private function scores(): array
    {
        ...
        $subjectAverages = $this->presenter->getSubjectAverages(
            $this->resource,
        );
        $subjectPositions = $this->presenter->getSubjectPositions(
            $this->resource,
        );
                ...
                $this->studentScores
                    ...
                    ->map(
                        fn($s) => [
                            ...
                            "score" => $s->score,
                            "subject_average" =>
                                $subjectAverages[$s->subject] ?? null,
                            "subject_position" =>
                                $subjectPositions[$s->subject] ?? null,
                        ],
                    )
                    ->values() ?? [],
    ...
    {
        return [
            "teachers" => collect($staff)
                ->filter(fn($t) => is_object($t))
                ->map(
                    fn($t) => [
